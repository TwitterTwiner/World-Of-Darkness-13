...
/mob/living/carbon/human/npc/proc/AssignSocialRole(datum/socialrole/S, dont_random = FALSE)
	...
	if(!dont_random)
		...
		var/color = random_eye_color()
		if(organ_eyes)
			organ_eyes.eye_color_left = color
			organ_eyes.eye_color_right = color
...
/mob/living/carbon/human/npc/bullet_act(obj/projectile/P, def_zone, piercing_hit = FALSE)
	. = ..()
	if(P)
		if(P.firer)
			for(var/mob/living/carbon/human/npc/NEPIC in oviewers(7, src))
